...
function resolveAnim(position, phase, spec) {
  ...
  if (!spec) {
    const def = phase === "enter" ? { preset: "slide", direction: autoDirection(position, phase), distance: defaultDistance(position, phase) } : { preset: "fade" };
    return {
      preset: def.preset,
...
function defaultDistance(position, phase) {
  ...
  if (position === "bottom-center") return phase === "enter" ? subtle : offscreen;
  return offscreen;
}
...
function beginExit(toastEl, container, position, userExitSpec) {
  ...
  const exitResolved = resolveAnim(position, "exit", userExitSpec != null ? userExitSpec : cfg.toast.animations.exit);
  ...
  onceAnimationEnd(toastEl, exitResolved.durationMs, () => {
    collapseAndRemove(container, position, exitResolved.durationMs, exitResolved.easing);
  });
}
function toast(opts) {
  var _a, _b, _c, _d, _e;
  ...
  const enterResolved = resolveAnim(pos, "enter", opts.animations && opts.animations.enter || cfg.toast.animations.enter);
...
export { modal_exports as Modal, alert, config, confirm, i18n_exports as i18n, question, toast, version };
